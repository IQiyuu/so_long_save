...
/*   Updated: 2022/11/20 17:58:14 by dgoubin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	find_way(char **map, t_coords *act, t_list **closed, int *args)
{
	...
	if ((!end && map[act->y][act->x + 1] != '1' && !is_in(*closed, act, 1, 0)))
		end = find_way(map, new_coords(act->x + 1, act->y), closed, args);
	if ((!end && map[act->y + 1][act->x] != '1' && !is_in(*closed, act, 0, 1)))
